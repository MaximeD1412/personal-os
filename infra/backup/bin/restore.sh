...
#
#   restore.sh --target DIR [--snapshot ID] [--into-postgres] [--read-data]
#              [--keep] [--dry-run]
#
...
if [ "$DRY_RUN" = 1 ]; then
  ...
  if [ "$INTO_POSTGRES" = 1 ]; then
    ...
    plan "dsn: postgresql://postgres:***@localhost:${PROBE_PORT}/${POSTGRES_DB}"
  fi
  exit 0
fi
...
docker exec -i --env PGPASSWORD="$PROBE_PASSWORD" "$PROBE_CONTAINER" \
  pg_restore --username postgres --dbname "$POSTGRES_DB" --no-owner --exit-on-error \
  <"$DUMP_PATH"
...
printf 'dsn: postgresql://postgres:%s@localhost:%s/%s\n' \
  "$PROBE_PASSWORD" "$PROBE_PORT" "$POSTGRES_DB"
